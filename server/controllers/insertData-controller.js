...
const promiseInsertProduct = async (product) => {
  try {
    ...
    let productCategoryInDb = await ProductCategory.findOne({
...
const promiseInsertBrandWithProductCategory = async (brandObj) => {
  try {
    ...
    let brandInDb = await Brand.create({
      title: brand,
      category: Array.from(productCategory),
      image: brandObj.image,
    });
    return brandInDb;
  } catch (e) {
    console.error(
      `Đã xảy ra lỗi tại hàm promiseInsertBrandWithProductCategory ${e}`
    );
  }
};
