...
const createProductCategory = async (productCategory) => {
  ...
  let existProductCategory = await ProductCategory.findOne({
  ...
  if (!existProductCategory) {
    ...
    return {
      ...
      code: result ? 201 : 500,
      devMsg: result
        ? "create product category thành công."
        : "create return null.",
      userMsg: result
...
const getProductCategoryById = async (_id) => {
  let productCategory = await ProductCategory.findById(_id).populate("brands");
  return {
...
const getProudctCategories = async (req) => {
  ...
  queryCommand = queryCommand.skip(skip).limit(limit).populate("brands");
...
const deleteProductCategoryById = async (pcid) => {
  ...
  return {
    ...
    userMsg: deletedProductCategory
      ? "Xóa danh mục thành công."
      : "Xóa danh mục chưa thành công.",
